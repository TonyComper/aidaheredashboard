...
async function resolveSerpPlaceIdsFromGooglePlaceId({
  ...
  // 1) Try direct place lookup first
  ...
  // 2) Fallback: local search by hotel name
...
const localResults = Array.isArray(localData?.local_results) ? localData.local_results : [];
...
/* --------------------------- HTTPS: resolveSerpDataIdForHotel --------------------------- */

exports.resolveSerpDataIdForHotel = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const cfgSnap = await db.ref(`hotels/${hotelCode}/config/reputation`).get();
      const cfg = cfgSnap.val() || {};

const googlePlaceId = safeTrim(cfg.googlePlaceId || "");
const hotelDisplayName = safeTrim(cfg.hotelDisplayName || "");
...
if (!googlePlaceId) {
  return res.status(400).json({ ok: false, error: "Missing config.reputation.googlePlaceId" });
}
...
await db.ref(`hotels/${hotelCode}/config/reputation`).update({
  ...
  serpResolveMethod: resolved.resolveMethod || "",
      ...
      console.error("resolveSerpDataIdForHotel error:", err);
      return res.status(500).json({ ok: false, error: String(err?.message || err) });
    }
  }
);
...
/* --------------------------- Reputation Phase 1 Builder --------------------------- */

async function buildReputationPhase1Report({ hotelCode, hotelDisplayName, reviews, apiKey }) {
  const nowMs = Date.now();
  ...
  function addTheme(themeId, r) {
    if (!themeAgg[themeId]) {
      themeAgg[themeId] = {
        themeId,
        title: THEME_META?.[themeId]?.title || themeId,
        bucket: THEME_META?.[themeId]?.bucket || "unknown",
        mentions: 0,
        mentionsLast7d: 0,
        mentionsPrior23d: 0,
  ...
  for (const r of norm) {
    const themes = detectThemesForReview(r.text);
  ...
  const themesArr = Object.values(themeAgg)
    .map((t) => {
      ...
      const severity = t.mentions ? Number((t.oneTwoStarMentions / t.mentions).toFixed(2)) : 0;
  ...
  const reviewBrief = sampleRecent.map((r) => ({ rating: r.rating, isoDate: r.isoDate, text: r.text }));
  ...
  if (apiKey) {
    const prompt = [
      "You are a hospitality reputation analyst.",
      "You will receive ONLY last-30-days written guest reviews (text) for one hotel.",
      "Return ONLY valid minified JSON. No markdown. No extra text.",
      "",
      "Answer using ONLY provided evidence:",
      "1) consistentComplaints (ranked bullets)",
      "2) risingTheme (what is increasing last 7d vs prior 23d)",
      "3) brandDamage (what damages perception the most and why)",
      "4) fixImmediately (max 8; low/no capex)",
      "5) requiresCapital (max 6 capex items)",
      "6) noiseVsSystemic (split: emotionalNoise vs systemicIssues)",
      "7) reputationRiskLevel = LOW|MEDIUM|HIGH (brief rationale)",
      "",
      "Also include recommendations: next7Days, next30Days, ownerLevelCapex (arrays of strings).",
      "",
      `Hotel: ${hotelDisplayName || hotelCode}`,
      `HotelCode: ${hotelCode}`,
      "",
      "Theme signals:",
    ...
    try {
      gpt = await openaiJson({ apiKey, model: "gpt-5.2", prompt, reasoningEffort: "low" });
      gptMeta = { ok: true };
    } catch (e) {
  ...
  const answers = gpt?.answers || {
    ...
    riskRationale: `Rule-based fallback: avgRating=${avgRating}, oneStarRatio=${Number(oneStarRatio.toFixed(2))}`,
  };

  const riskFinal = ["LOW", "MEDIUM", "HIGH"].includes(answers.reputationRiskLevel) ? answers.reputationRiskLevel : riskRule;
  ...
  return {
    ...
    sampleRecent,
    answers,
    recommendations: gpt?.recommendations || { next7Days: [], next30Days: [], ownerLevelCapex: [] },
    ops: {
      generatedAtMs: nowMs,
...
/* --------------------------- Reputation Phase 2 Builder (Trend + Heatmap) --------------------------- */

async function buildReputationPhase2Report({ hotelCode, hotelDisplayName, reviews, apiKey, hotelTimeZone = "America/New_York" }) {
  const nowMs = Date.now();
  const windowMs = 30 * 24 * 60 * 60 * 1000;
  const startMs = nowMs - windowMs;
  ...
  // Build day list (last 30 days keys, in hotel TZ)
  const dayKeys = [];
  for (let i = 29; i >= 0; i--) {
    const ms = nowMs - i * 24 * 60 * 60 * 1000;
    dayKeys.push(dayKeyFromMs(ms, hotelTimeZone));
  }

  // Heatmap: theme -> { dayKey -> count }
  const heatmap = {}; // themeId -> { dayKey: count }
  const themeAgg = {}; // themeId -> stats
  ...
  for (const r of norm) {
    ...
    const themes = detectThemesForReview(r.text);
    const dk = dayKeyFromMs(r.dateMs, hotelTimeZone);
    ...
    for (const th of themes) {
      ...
      if (!themeAgg[th]) {
        themeAgg[th] = {
          themeId: th,
          title: THEME_META?.[th]?.title || th,
          bucket: THEME_META?.[th]?.bucket || "unknown",
          mentions: 0,
          mentionsLast7d: 0,
          mentionsPrior23d: 0,
          oneTwoStarMentions: 0,
  ...
  const themesArr = Object.values(themeAgg)
    ...
    .sort((a, b) => {
      // Phase 2: prioritize rising + severe
      if (b.trend7dVsPrior !== a.trend7dVsPrior) return b.trend7dVsPrior - a.trend7dVsPrior;
      if (b.severity !== a.severity) return b.severity - a.severity;
      return b.mentions - a.mentions;
    });

  // Trend detection (Phase 2): pick top “rising” themes with minimum evidence
  const risingThemes = themesArr
    .filter((t) => (t.mentionsLast7d || 0) >= 2) // minimum evidence
    .filter((t) => (t.trend7dVsPrior || 0) >= 0.05) // tune threshold later
    .slice(0, 8)
    .map((t) => ({
      themeId: t.themeId,
      title: t.title,
  ...
  // Heatmap compact output (only top 10 themes to keep Safari fast)
  const topThemeIds = topThemes.map((t) => t.themeId);
  const heatmapTop = {};
  for (const th of topThemeIds) {
    heatmapTop[th] = {};
    for (const dk of dayKeys) heatmapTop[th][dk] = Number(heatmap?.[th]?.[dk] || 0);
  }
  ...
  // Optional GPT synthesis for Phase 2 “manager summary”
  let gpt = null;
  let gptMeta = { ok: false, reason: "not_called" };

  if (apiKey && totalTextReviews > 0) {
    const prompt = [
      "You are a hospitality reputation analyst for a hotel operator.",
...
return {
  ...
  hotelCode,
  hotelDisplayName: hotelDisplayName || "",
  window: { label: "Last 30 days", startMs, endMs: nowMs, timeZone: hotelTimeZone },
  counts: {
    totalTextReviews,
    avgRating,
    ratingBuckets: buckets,
  ...
  heatmap: {
    dayKeys,
    themes: heatmapTop,
    themeMeta: Object.fromEntries(topThemes.map((t) => [t.themeId, { title: t.title, bucket: t.bucket }])),
  ...
  narrative: gpt
    ? {
        ...
        recommendedOwnerActions7d: Array.isArray(gpt.recommendedOwnerActions7d) ? gpt.recommendedOwnerActions7d : [],
        recommendedOwnerActions30d: Array.isArray(gpt.recommendedOwnerActions30d) ? gpt.recommendedOwnerActions30d : [],
        riskLevel: riskLevelFinal,
        riskRationale:
    ...
    : {
        ...
        riskLevel: riskLevelFinal,
        riskRationale: `Rule fallback: avgRating=${avgRating}, oneStarRatio=${Number(oneStarRatio.toFixed(2))}`,
      },
  ops: {
    generatedAtMs: nowMs,
    source: "serpapi_google_maps_reviews",
...
/* --------------------------- REBUILD Reputation Phase 1 --------------------------- */

exports.rebuildReputationPhase1 = onRequest(
  { region: "us-central1", secrets: [OPENAI_API_KEY], timeoutSeconds: 300, memory: "1GiB" },
  async (req, res) => {
    setCors(res);
    if (req.method === "OPTIONS") return res.status(204).send("");
    ...
    try {
      ...
      const hotelCode = normalizeHotelCode(hotelCodeRaw);
      if (!hotelCode) return res.status(400).json({ ok: false, error: "hotelCode query param required" });
      ...
      const reviewsSnap = await db.ref(`hotels/${hotelCode}/reputation/serpapi/reviews`).get();
      ...
      const report = await buildReputationPhase1Report({
        hotelCode,
        hotelDisplayName,
      ...
      const outPath = `hotels/${hotelCode}/insights/reputationPhase1`;
      ...
      console.error("rebuildReputationPhase1 error:", err);
      return res.status(500).json({ ok: false, error: String(err?.message || err) });
    }
  }
);

/* --------------------------- HTTPS: getReputationPhase1 --------------------------- */

exports.getReputationPhase1 = onRequest({ region: "us-central1" }, async (req, res) => {
  setCors(res);
  if (req.method === "OPTIONS") return res.status(204).send("");

  try {
    const hotelCodeRaw = safeTrim(req.query?.hotelCode || "");
    if (!hotelCodeRaw) return res.status(400).json({ ok: false, error: "hotelCode required" });

    const hotelCode = normalizeHotelCode(hotelCodeRaw);

    const snap = await db.ref(`hotels/${hotelCode}/insights/reputationPhase1`).get();
    const val = snap.val();

    if (!val) return res.status(404).json({ ok: false, error: "No reputationPhase1 report found" });

    return res.json({ ok: true, hotelCode, data: val });
  } catch (err) {
    console.error("getReputationPhase1 error:", err);
    return res.status(500).json({ ok: false, error: String(err?.message || err) });
  }
});

/* --------------------------- REBUILD Reputation Phase 2 --------------------------- */
/**
 * This is what your dashboard Refresh button should call.
 * It reads SerpAPI reviews already stored under:
 *   hotels/{hotelCode}/reputation/serpapi/reviews
 * Then it writes:
 *   hotels/{hotelCode}/insights/reputationPhase2/latest
 *   hotels/{hotelCode}/insights/reputationPhase2/history/{YYYY-MM-DD}
 */
exports.rebuildReputationPhase2 = onRequest(
  { region: "us-central1", secrets: [OPENAI_API_KEY], timeoutSeconds: 300, memory: "1GiB" },
  async (req, res) => {
    setCors(res);
    if (req.method === "OPTIONS") return res.status(204).send("");
    ...
    try {
      ...
      const hotelCode = normalizeHotelCode(hotelCodeRaw);
      if (!hotelCode) return res.status(400).json({ ok: false, error: "hotelCode required" });

      const cfgSnap = await db.ref(`hotels/${hotelCode}/config/reputation`).get();
      const cfg = cfgSnap.val() || {};
      const hotelDisplayName = safeTrim(cfg.hotelDisplayName || "");
      const hotelTimeZone = safeTrim(cfg.timeZone || "") || (await db.ref(`hotels/${hotelCode}/config/timeZone`).get()).val() || "America/New_York";
      ...
      const report = await buildReputationPhase2Report({
        hotelCode,
        hotelDisplayName,
        reviews,
        apiKey,
      ...
      const nowKey = dayKeyFromMs(Date.now(), hotelTimeZone);

      const latestPath = `hotels/${hotelCode}/insights/reputationPhase2/latest`;
      const histPath = `hotels/${hotelCode}/insights/reputationPhase2/history/${sanitizeDbKey(nowKey)}`;
      ...
      return res.json({
        ...
        risingThemes: Array.isArray(report?.trend?.risingThemes) ? report.trend.risingThemes.length : 0,
      });
    } catch (err) {
      console.error("rebuildReputationPhase2 error:", err);
      return res.status(500).json({ ok: false, error: String(err?.message || err) });
    }
  }
);

/* --------------------------- HTTPS: getReputationPhase2 --------------------------- */

exports.getReputationPhase2 = onRequest({ region: "us-central1" }, async (req, res) => {
  setCors(res);
  if (req.method === "OPTIONS") return res.status(204).send("");
  ...
  try {
    ...
    const snap = await db.ref(`hotels/${hotelCode}/insights/reputationPhase2/latest`).get();
    const val = snap.val();

    if (!val) return res.status(404).json({ ok: false, error: "No reputationPhase2 report found" });

    // Keep response small & Safari-friendly
    return res.json({ ok: true, hotelCode, data: val });
  } catch (err) {
    console.error("getReputationPhase2 error:", err);
    return res.status(500).json({ ok: false, error: String(err?.message || err) });
  }
});

/* --------------------------- Scheduled: buildReputationPhase2AllHotels --------------------------- */
/**
 * Nightly job:
 * - sync SerpAPI reviews (last 30 days text only) for each active hotel config
 * - rebuild Phase 1
 * - rebuild Phase 2 and write latest + history snapshot
 */
exports.buildReputationPhase2AllHotels = onSchedule(
  {
    region: "us-central1",
    schedule: "25 3 * * *",
    ...
    for (const hotelCodeRaw of codes) {
      ...
      try {
        ...
        const hotelTimeZone =
          ...
          (await db.ref(`hotels/${hotelCode}/config/timeZone`).get()).val() ||
          "America/New_York";

        // --- refresh SerpAPI reviews in DB
        ...
        await db.ref(`${basePath}/meta`).set({
          ...
          cutoffMs,
          cutoffIso: new Date(cutoffMs).toISOString(),
          note: "Nightly scheduled sync for Phase 1 + Phase 2",
        });

        // --- read stored reviews back from DB
        const reviewsSnap = await db.ref(`${basePath}/reviews`).get();
        const reviewsObj = reviewsSnap.val() || {};
        const reviewsArr = Object.values(reviewsObj);

        // --- rebuild Phase 1 from stored reviews
        const phase1Report = await buildReputationPhase1Report({
          hotelCode,
          hotelDisplayName,
          reviews: reviewsArr,
          apiKey: apiKeyOpenAI,
        ...
        await db.ref(`hotels/${hotelCode}/insights/reputationPhase1`).set(phase1Report);

        // --- rebuild Phase 2 from stored reviews
        const report = await buildReputationPhase2Report({
          hotelCode,
          hotelDisplayName,
          reviews: reviewsArr,
        ...
        console.error("buildReputationPhase2AllHotels error:", hotelCode, String(e?.message || e));
      }
    }
  }
);

/* --------------------------- HTTPS: setHotelReputationFields (ADMIN) --------------------------- */

exports.setHotelReputationFields = onRequest({ region: "us-central1" }, async (req, res) => {
  setCors(res);
  if (req.method === "OPTIONS") return res.status(204).send("");
  ...
  try {
    ...
    if (hotelDisplayName !== undefined) updates.hotelDisplayName = safeTrim(hotelDisplayName);
    if (timeZone !== undefined) updates.timeZone = safeTrim(timeZone);
    if (active !== undefined) updates.active = active !== false;

    await db.ref(`hotels/${hotelCode}/config/reputation`).update(updates);

    return res.json({
      ok: true,
      hotelCode,
      path: `hotels/${hotelCode}/config/reputation`,
      updates,
    });
  } catch (err) {
    console.error("setHotelReputationFields error:", err);
    return res.status(500).json({ ok: false, error: String(err?.message || err) });
  }
});